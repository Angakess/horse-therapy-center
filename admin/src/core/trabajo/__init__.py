...
class Trabajo(db.Model):
    ...
    propuestra_trabajo_institucional = db.Column(
        db.Enum(
            "Hipoterapia",
            "Monta terapéutica",
    ...
    condicion = db.Column(
        db.Enum("Regular", "De baja", name="condicion_enum"), nullable=False
    )

    sede = db.Column(db.Enum("CASJ", "HLP", "Otro", name="sede_enum"), nullable=False)
    ...
    profesor_terapeuta = db.relationship(
        "Equipo",
        back_populates="profesor_terapeuta_trabajo",
        foreign_keys=[profesor_terapeuta_id],
    )

    conductor_id = db.Column(db.Integer, db.ForeignKey("equipos.id"))
    conductor = db.relationship(
        "Equipo", back_populates="conductor_trabajo", foreign_keys=[conductor_id]
    )

    auxiliar_pista_id = db.Column(db.Integer, db.ForeignKey("equipos.id"))
    auxiliar_pista = db.relationship(
        "Equipo",
        back_populates="auxiliar_pista_trabajo",
...
def get_propuestas_cant_solicitadas(propuesta):
    cant = (
        db.session.query(func.count(JinetesAmazonas.id))
        .join(Trabajo, Trabajo.id == JinetesAmazonas.trabajo_id)
        .filter(Trabajo.propuestra_trabajo_institucional == propuesta)
        .scalar()
    )
